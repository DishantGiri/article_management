...
CREATE TABLE `User` (
    ...
    `role` ENUM('ADMIN', 'LINKER', 'WRITER', 'TEAM_LEAD') NOT NULL,
...
CREATE TABLE `Site` (
    ...
    `productType` ENUM('NUTRA', 'ECOM') NOT NULL,
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
...
CREATE TABLE `Category` (
    ...
    UNIQUE INDEX `Category_name_siteId_key`(`name`, `siteId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `Notification` (
    ...
    `type` ENUM('ARTICLE_SUGGESTION', 'LINK_ISSUE', 'APPROVAL_GRANTED') NOT NULL,
...
-- AddForeignKey
ALTER TABLE `SpecialApproval` ADD CONSTRAINT `SpecialApproval_articleId_fkey` FOREIGN KEY (`articleId`) REFERENCES `Article`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `LinkLog` ADD CONSTRAINT `LinkLog_productId_fkey` FOREIGN KEY (`productId`) REFERENCES `Product`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `LinkGeo` ADD CONSTRAINT `LinkGeo_linkLogId_fkey` FOREIGN KEY (`linkLogId`) REFERENCES `LinkLog`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
