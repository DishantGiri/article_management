...
export default function EditLinkModal({ isOpen, onClose, onSuccess, link }: EditLinkModalProps) {
  ...
  // Single-shared mode fallback entries
  ...
  useEffect(() => {
    if (isOpen && link) {
      ...
      fetch("/api/geos")
        .then((r) => r.json())
        .then((data) => {
          setDbGeos(Array.isArray(data) ? data.map((g: any) => g.code || g.name || g) : []);
        })
  ...
  const handleSubmit = async () => {
    ...
    if (useCountrySpecificLinks) {
      ...
      if (countryLinks.some((c) => !c.affiliateName.trim() || !c.affiliateLink.trim())) {
        setError("Affiliate Network and Affiliate Link are required for all country entries.");
        return;
      }
      if (countryLinks.some((c) => !isValidUrl(c.affiliateLink))) {
        setError("All country affiliate links must be valid URLs starting with http:// or https://");
        return;
      }
    } else {
      if (affiliateEntries.length === 0) {
        setError("Please configure at least one affiliate network entry.");
        return;
      }
      ...
      if (affiliateEntries.some((e) => !isValidUrl(e.affiliateLink))) {
        setError("All affiliate links must be valid URLs starting with http:// or https://");
        return;
      }
    }
    ...
    try {
      ...
      toast.success(
        useCountrySpecificLinks
          ? `Successfully saved product link with ${countryLinks.length} country link(s)!`
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-bold text-white tracking-tight flex items-center gap-2">
                <span>Edit Link Log #{link.id}</span>
                {selectedProduct && (
