...
export default function HistoryPage() {
  ...
  const filtered = history.filter((record) => {
    // Type Filter
    if (typeFilter) {
      if (typeFilter === "LINK_ISSUE") {
        if (record.actionType !== "LINK_FLAGGED") return false;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-1.5 p-1 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-2xs">
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-xl text-xs font-bold transition flex items-center gap-1.5 cursor-pointer ${actionFilter === "LINK_FLAGGED"
                ? "bg-rose-600 text-white shadow-2xs"
                : "text-slate-600 dark:text-slate-300 hover:text-rose-700 dark:hover:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/40"
              }`}
          >
            ...
            {history.filter((h) => h.actionType === "LINK_FLAGGED").length > 0 && (
              <span
                className={`px-1.5 py-0.2 rounded-full text-[10px] font-extrabold ${actionFilter === "LINK_FLAGGED" ? "bg-white text-rose-700" : "bg-rose-100 dark:bg-rose-950/80 text-rose-700 dark:text-rose-300"
                  }`}
              >
                {history.filter((h) => h.actionType === "LINK_FLAGGED").length}
              </span>
            )}
          </button>
        </div>
                          ...
                          {record.actionType === "LINK_FLAGGED" ? (
                            <div className="p-2.5 bg-rose-50/80 dark:bg-rose-950/40 rounded-xl border border-rose-200 dark:border-rose-900/60 shadow-2xs space-y-1">
                              ...
                              <p className="text-xs text-rose-950 dark:text-rose-100 font-medium leading-relaxed">
                                {record.notes?.replace(/^\[Flagged by [^\]]+\]:\s*/i, "") || "Issue reported on link."}
                              </p>
                            </div>
                          ) : (
                            !isRedoSuggestion &&
