...
import { Plus, X, Trash2, Tag, Layers, AlertCircle, ShieldAlert } from "lucide-react";
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-[1600px] mx-auto min-h-screen bg-[#FAF9F5] dark:bg-slate-950 text-[#4A4A4A] dark:text-slate-100 transition-colors">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-2xl font-bold text-[#4A4A4A] dark:text-slate-100 tracking-tight flex items-center gap-2.5">
            <Layers className="w-6 h-6 text-[#6D8196]" />
            Product Types
          </h1>
      ...
      {loading ? (
        <div className="py-16">
          <LoadingScreen
            message="Loading product types..."
            subtext="Retrieving product type classifications and site links"
            size="md"
          />
        </div>
      ) : categories.length === 0 ? (
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-[#CBCBCB]/60 dark:border-slate-800 p-16 text-center shadow-xs">
          ...
          <p className="text-[#737373] dark:text-slate-400 font-medium">No product types found.</p>
          {canManage && (
            <button
              onClick={openAddModal}
          ...
          <div className="flex flex-wrap gap-3">
            {categories.map((cat) => (
              <button
                ...
                className="group flex flex-col items-start px-4 py-3 rounded-xl bg-[#FAF9F5] dark:bg-slate-800/70 border border-[#CBCBCB] dark:border-slate-700 hover:bg-white dark:hover:bg-slate-800 hover:border-[#6D8196] hover:shadow-sm transition-all text-left w-[220px] cursor-pointer"
              >
                <div className="flex items-center gap-1.5 w-full">
                  <Layers className="w-3.5 h-3.5 text-[#6D8196] shrink-0" />
                  <span className="text-[14px] font-bold text-[#4A4A4A] dark:text-slate-200 group-hover:text-[#6D8196] dark:group-hover:text-sky-300 transition-colors truncate">
                    {cat.name}
                  </span>
                </div>
                <div className="mt-2 flex gap-3 text-[11px] font-medium text-[#737373] dark:text-slate-400">
            ...
            <form onSubmit={handleAddCategory} className="p-5 bg-white dark:bg-slate-900 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Ecom, Nutra, Digital"
